...
class SolverWrapper(object):
  ...
  def __init__(self, network, imdb, roidb, valroidb, output_dir, tbdir, pretrained_model=None, image_channels=3):
    self.net = network
    self.imdb = imdb
    self.roidb = roidb
  ...
  def find_previous(self):
    ...
    print("found previous: ", lsf, nfiles, sfiles)
  ...
  def initialize(self):
    ...
    if self.image_channels == 3:
      ...
      self.net.load_pretrained_cnn_rgba(torch.load(self.pretrained_model))
  ...
  def train_model(self, max_iters):
    ...
    while iter < max_iters + 1:
      ...
      blobs = self.data_layer.forward(rgba=(self.image_channels == 4))
      ...
      if iter == 1 or now - last_summary_time > cfg.TRAIN.SUMMARY_INTERVAL:
        ...
        blobs_val = self.data_layer_val.forward(rgba=(self.image_channels == 4))
      ...
      if iter % (cfg.TRAIN.DISPLAY) == 0:
        ...
        print('speed: {:.3f}s / iter'.format(lib.utils.timer.timer.average_time()))
...
def train_net(network, imdb, roidb, valroidb, output_dir, tb_dir,
              ...
              max_iters=40000, image_channels=3):
  """Train a Faster R-CNN network."""
  roidb = filter_roidb(roidb)
  valroidb = filter_roidb(valroidb)

  sw = SolverWrapper(network, imdb, roidb, valroidb, output_dir, tb_dir,
                     pretrained_model=pretrained_model, image_channels=image_channels)
